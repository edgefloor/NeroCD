...
Unknown or disabled identities cannot create sessions. State, browser binding, nonce, token verification, expiry, and replay failures fail closed. Provider logout, identity removal, SCIM, SAML, LDAP, automatic signup, claim-driven role or group mapping, multiple issuers, and provider administration UI are outside this first release. NeroCD does not currently expose a user-disable or external-identity removal command. When offboarding, a system administrator must list and revoke the user's active sessions through `GET /api/v1/sessions` and `POST /api/v1/sessions/revoke`, then disable access at the identity provider. Removing or disabling the provider-side account alone does not revoke an already issued NeroCD session.
