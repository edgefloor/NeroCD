...
compose(){
  ...
  NEROCD_RUNTIME_IMAGE="$image" NEROCD_RUNNER_IMAGE="$runner_image" NEROCD_GIT_IMAGE="$git_image" NEROCD_FIXTURE_A="$fixture_a_ref" NEROCD_FIXTURE_B="$fixture_b_ref" NEROCD_FIXTURE_C="$fixture_c_ref" NEROCD_DOCKER_GID="$socket_gid" docker compose -p "$project" "${files[@]}" "$@"
}
...
compose_project="target_$suffix"; env=$(jq -cn --arg s "$service_id" --arg cp "$compose_project" --arg host host.docker.internal --arg cidr "$host_ip/32" '{service_id:$s,name:"runtime",runner_selector:["compose-runtime"],compose_project:$cp,timeout_seconds:40,rollback_safe:true,health_policy:{url:"http://host.docker.internal:18080/cgi-bin/health",allowed_hosts:[$host],allowed_cidrs:[$cidr],allowed_ports:[18080],allow_http:true,interval_seconds:1,timeout_seconds:10,expected_status:200},secret_bindings:[{name:"git",provider:"runner_file",reference:"cred_git_deploy",target:"env:GIT_SSH_KEY",required:true,version:"v1",fingerprint:"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},{name:"application",provider:"runner_file",reference:"app_secret",target:"file:app_secret",required:true,version:"v1",fingerprint:"sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"}]}' ); code=$(http POST "$base/api/v1/environments" "$admin" "$env" "$dir/env.json"); [[ "$code" == 201 ]] || fail 'environment create failed'; env_id=$(jq -er .id "$dir/env.json")
