...
health_network="${project}-health" health_host="runtime-health-$suffix" health_url="http://${health_host}:8080/cgi-bin/health"
...
compose_project="target_$suffix"; env=$(jq -cn --arg s "$service_id" --arg cp "$compose_project" --arg url "$health_url" --arg host "$health_host" --arg cidr "$health_cidr" '{service_id:$s,name:"runtime",runner_selector:["compose-runtime"],compose_project:$cp,timeout_seconds:40,rollback_safe:true,health_policy:{url:$url,allowed_hosts:[$host],allowed_cidrs:[$cidr],allowed_ports:[8080],allow_http:true,interval_seconds:1,timeout_seconds:10,expected_status:200},secret_bindings:[{name:"git",provider:"runner_file",reference:"cred_git_deploy",target:"env:GIT_SSH_KEY",required:true,version:"v1",fingerprint:"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},{name:"application",provider:"runner_file",reference:"app_secret",target:"file:app_secret",required:true,version:"v1",fingerprint:"sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"}]}' ); code=$(http POST "$base/api/v1/environments" "$admin" "$env" "$dir/env.json"); [[ "$code" == 201 ]] || fail 'environment create failed'; env_id=$(jq -er .id "$dir/env.json")
