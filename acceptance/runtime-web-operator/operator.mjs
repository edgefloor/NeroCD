...
try {
  ...
  await writeFile(resultPath, JSON.stringify({ deployment_id: deploymentID, phase, polling: true, reload: true, history: true, mobile: true, runner_calls: 0, page_errors: 0, console_nonresource_errors: 0, console_resource_diagnostics: consoleResourceDiagnostics.length, csp_inline_script_blocked: true, csp_external_script_blocked: true }, null, 2), { mode: 0o600 });
} finally {
  await browser.close();
}
