  ...
  return (
    <>
      <div className="grid gap-2 p-3 md:hidden">
        {runs.map((run) => (
          <div key={run.id} className="grid gap-2 rounded-lg border border-border bg-card p-3">
            ...
            <div className="flex gap-2 text-xs text-muted-foreground">
              <span className="rounded bg-muted px-2 py-1">{formatDuration(run)}</span>
              <span className="truncate rounded bg-muted px-2 py-1 font-mono">{run.id}</span>
            </div>
            <div className="flex flex-wrap gap-2">
              {onOpenLogs ? (
                <Button variant="outline" size="sm" className="h-8" onClick={() => onOpenLogs(run.id)}>
      ...
      <div className="hidden md:block">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="text-xs font-medium">Run</TableHead>
