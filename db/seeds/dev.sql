...
VALUES
    ('tpl_patch', 'proj_platform', 'Patch Linux Fleet', 'ansible', '{"type":"ansible","inputs":{"playbook":"patch.yml"},"repository":{"id":"repo_platform_runbooks","ref":"main","path":"ansible"},"process":{"command":["ansible-playbook","patch.yml"],"timeout_seconds":1800},"artifacts":[{"name":"patch-report","path":"reports/patch.json","required":false}],"secrets":[{"name":"ansible-vault","provider":"env","reference":"NEROCD_DEV_ANSIBLE_VAULT","target":"env:ANSIBLE_VAULT_PASSWORD","classification":"development"}]}'::jsonb, '{"steps":[{"id":"checkout","name":"Checkout runbooks","run_spec":{"type":"shell","inputs":{"command":"git checkout"}}},{"id":"patch","name":"Patch fleet","depends_on":["checkout"],"requires_ack":true,"run_spec":{"type":"ansible","inputs":{"playbook":"patch.yml"}}}]}'::jsonb, ARRAY['linux', 'prod'], true),
    ...
    ('tpl_rotate', 'proj_security', 'Rotate Service Tokens', 'shell', '{"type":"shell","inputs":{"command":"./rotate-tokens.sh"},"repository":{"id":"repo_security_runbooks","ref":"main","path":"tokens"},"process":{"command":["./rotate-tokens.sh"],"timeout_seconds":600},"secrets":[{"name":"token-admin","provider":"env","reference":"NEROCD_DEV_TOKEN_ADMIN","target":"env:TOKEN_ADMIN","classification":"development"}]}'::jsonb, '{"steps":[{"id":"checkout","name":"Checkout security runbooks","run_spec":{"type":"shell","inputs":{"command":"git checkout"}}},{"id":"rotate","name":"Rotate tokens","depends_on":["checkout"],"requires_ack":true,"run_spec":{"type":"shell","inputs":{"command":"./rotate-tokens.sh"}}}]}'::jsonb, ARRAY['secure'], true)
ON CONFLICT (id) DO NOTHING;
...
VALUES
    ...
    ('run_002', 'proj_security', 'tpl_rotate', '{"type":"shell","inputs":{"command":"./rotate-tokens.sh"},"repository":{"id":"repo_security_runbooks","ref":"main","path":"tokens"},"process":{"command":["./rotate-tokens.sh"],"timeout_seconds":600},"secrets":[{"name":"token-admin","provider":"env","reference":"NEROCD_DEV_TOKEN_ADMIN","target":"env:TOKEN_ADMIN","classification":"development"}]}'::jsonb, '{"steps":[{"id":"checkout","name":"Checkout security runbooks","run_spec":{"type":"shell","inputs":{"command":"git checkout"}}},{"id":"rotate","name":"Rotate tokens","depends_on":["checkout"],"requires_ack":true,"run_spec":{"type":"shell","inputs":{"command":"./rotate-tokens.sh"}}}]}'::jsonb, ARRAY['secure'], 'waiting_approval', 'usr_bootstrap', now() - interval '5 minutes', NULL)
ON CONFLICT (id) DO NOTHING;
