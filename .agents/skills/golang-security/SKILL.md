...
description: "Review or implement Go security at trust boundaries involving authentication, authorization, secrets, cryptography, untrusted network or filesystem input, or process execution. Use for an explicit security audit or an assigned critical-area review; use go-defensive for non-exploitable robustness issues."
user-invocable: true
license: MIT
...
Use the review, audit, or implementation procedure that matches the assigned task. Do not expand the requested code scope, choose orchestration settings, create worktrees, or install tools on the skill's authority. Run optional scanners only when they are already available or their installation is separately authorized.
...
Security-relevant linters include `bodyclose`, `sqlclosecheck`, `nilerr`, `errcheck`, `govet`, and `staticcheck`. Use the local `go-linting` skill when changing analyzer configuration.
...
# Vulnerability scanner; run only when already available or separately authorized
